...
interface BuildWorktreeOptionsArgs {
  ...
  /** Unarchived threads of the selected project, for row previews. */
...
/**
 * Pure enrichment over server-discovered rows: the server already owns
 * filtering, merging, availability, ownership, and within-host order. This
 * helper only attaches machine names and recent thread previews — it never
 * drops or reorders rows, so a worktree without any active thread stays
 * visible.
 */
export function buildWorktreeOptions(
  ...
  return {
    ...
    failures: args.failures.map((failure) => ({
      ...
      message:
        failure.code === "host_offline" ? "Machine is offline" : failure.message,
    })),
  };
}
