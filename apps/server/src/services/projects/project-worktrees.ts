...
function mergeHostWorktrees(args: MergeHostWorktreesArgs): ProjectWorktree[] {
  ...
  const sourceCanonicalPath = canonicalByStoredPath.get(args.sourcePath) ?? null;
  ...
  const orderedEnvironments = [...args.environments].sort(
    (left, right) =>
      Number(isReusableEnvironment(right)) - Number(isReusableEnvironment(left)),
  );
  for (const environment of orderedEnvironments) {
    ...
    if (canonicalPath !== null && !environmentByCanonicalPath.has(canonicalPath)) {
  ...
  rows.sort((left, right) => {
    const environmentRank =
      Number(right.environmentId !== null) - Number(left.environmentId !== null);
...
export async function discoverProjectWorktrees(
  ...
  const environments = listProjectEnvironmentsWithPaths(deps.db, args.projectId);
